...
singleton_list = [8,] # list: A list with only one element
singleton_set = {8,}# set: A set with only one element
singleton_tuple = (8, ) # tuple: A tuple with only one element
...
a_truthy_tuple = (5, ) # tuple: a tuple but when passed to bool function should return True
...
if reversed == True : # some iterables are not reversible why?
    int_iterable_reversed = int_iterable.reverse() # list: the int_iterable reversed use the reversed function
else: # in that case sort it in ascending order and reverse it
    int_iterable_reversed = int_iterable_sorted_desc #list

if type(some_collection) == list: # some collections are not indexable why?
...
if type(some_collection) == tuple: # some collections are not slicable
...
if type(some_collection) == list: # some collections are not ordered
...
if type(another_iterable) == list : # some iterables are not ordered
    all_concat = "-".join(another_iterable) # str: concatenate all the strings in string_iterable with '-' in between
else: # in that case sort them and concatenate
    all_concat = "-".join(sorted(another_iterable))
